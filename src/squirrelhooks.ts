...
function checkSquirrelHooks(): boolean {
    ...
    if (cmd === '--squirrel-install' || cmd === '--squirrel-updated') {
        runUpdateExe(['--createShortcut=' + target]).then(() => {
            ...
            // same for 'RhubarbVR (Riot) which is old now too (we have to try to delete both because
            // we don't know what version we're updating from, but of course we do know this version
            // is 'RhubarbVR' so the two old ones are all safe to delete).
            const appDataDir = process.env.APPDATA;
            if (!appDataDir) return;
            const oldStartMenuLink = path.join(
                appDataDir, 'Microsoft', 'Windows', 'Start Menu', 'Programs', 'RhubarbVR', 'RhubarbVR (Riot).lnk',
            );
            return fsProm.unlink(oldStartMenuLink).catch(() => {});
        }).then(() => {
            const oldDesktopShortcut = path.join(app.getPath('desktop'), 'RhubarbVR (Riot).lnk');
